...
export default function MessageBubble({
  ...
  const isLong = (content?.length || 0) > 600 || ((content?.match(/\n/g)?.length || 0) > 6);
  ...
  return (
    <motion.div
      ...
      className={
        ...
        (isUser ? 'ml-auto mr-1 md:mr-2 bg-primary text-primary-foreground rounded-br-sm' : 'mr-auto bg-muted rounded-bl-sm')
      }
    >
          ...
          {Array.isArray(imageUrls) && imageUrls.length > 0 ? (
            <div className="mb-2 grid grid-cols-2 sm:grid-cols-3 gap-2 w-full">
              {imageUrls.map((src, idx) => (
                <img
                  ...
                  src={src}
                  alt={`attachment-${idx}`}
                  className="rounded-lg max-h-40 object-cover w-full max-w-full block"
                />
              ))}
            </div>
          ) : imageUrl ? (
            <div className="mb-2 w-full overflow-hidden">
              <img
                src={imageUrl}
                alt="attachment"
                className="rounded-lg max-h-60 w-full h-auto max-w-full object-contain bg-black/5 dark:bg-white/5"
              />
            </div>
          ) : null}
      ...
      <button
        ...
        className={'absolute bottom-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 rounded-md border cursor-pointer bg-white text-black dark:bg-black dark:text-white'}
        onClick={handleCopy}
      >
        {copied ? (
